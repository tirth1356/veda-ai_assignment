...
If generating Multiple Choice Questions (or any question requiring selection), you MUST write the options directly into the \`questionText\` (e.g., "What is 2+2? (A) 3 (B) 4 (C) 5 (D) 6"). Do NOT ask the student to "select from the following" without actually listing the options.

CRITICAL INSTRUCTION FOR QUESTION COUNTS AND UNIQUENESS:
You MUST generate EXACTLY the requested number of questions. Ensure every single question is completely unique. Do NOT generate duplicate or extremely similar questions.
...
3. ABSOLUTE CRITICAL CHECK: If any question is a Multiple Choice Question (or asks the student to select a correct statement), ensure the options (A, B, C, D) are explicitly listed in the \`questionText\`. Rewrite it to include options if they are missing.
4. Output a single, valid JSON object matching the exact input structure. Do NOT change question counts or marks.`;
